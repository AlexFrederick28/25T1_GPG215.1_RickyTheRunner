...
public class PageChanger : MonoBehaviour
{
    ...
    void Start()
    {
        StartCoroutine(DeactivateObjects()); // turn off all pages initially

        pageNumber = 0; // set page to the start
        SetPageToLoadActive(); // load it
    }
    ...
    public void LoadPreviousPage()
    {
        if (pageNumber > 0)
        {
            pageNumber--; // set target previous page
            StartCoroutine(DeactivateObjects()); // turn off everything
            SetPageToLoadActive(); // load page
        }
